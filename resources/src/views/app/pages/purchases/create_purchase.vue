<template>
                      ...
                      <tbody>
                          ...
                          <td>
                              ...
                              <b-input-group>
                                  ...
                                  <input class="form-control" @keyup="Verified_Qty(detail, detail.detail_id)" :min="0.00" v-model.number="detail.quantity">
                          ...
                          <td>
                              <b-form-group>
                              <b-form-input type="date"  v-model="detail.exp_date"></b-form-input>
                              </b-form-group>
                          </td>
    ...
    <input
        ...
        @input="updateSubtotal(detail, true)"
        step="0.01"
        @blur="updateSubtotal(detail)"
    >
</td>

                          <td>
                              <i v-if="currentUserPermissions && currentUserPermissions.includes('edit_product_purchase')" @click="Modal_Updat_Detail(detail)" class="i-Edit text-25 text-success"></i>
  ...
  export default {
    ...
    methods: {
      ...
      updateSubtotal(detail, isManual = false) {
    // Parse subtotal as a float for precision
    let parsedSubtotal = parseFloat(detail.subtotal);

    // If subtotal is not a number, return early to avoid issues
    if (isNaN(parsedSubtotal)) {
        return;
    }

    // If the update is manual, do not perform any further calculations
    if (isManual) {
        // Round the subtotal to 2 decimal places to avoid floating-point issues
        detail.subtotal = parsedSubtotal.toFixed(2);
        return;
    }

    // Calculate the total tax amount
    const totalTax = parseFloat((detail.taxe * detail.quantity).toFixed(2));

    // Subtract the total tax from the subtotal
    const result = parsedSubtotal - totalTax;

    // Calculate the new net unit cost
    const newNetUnitCost = parseFloat((result / detail.quantity).toFixed(2));

    // Update the detail object with the new unit cost, ensuring precision
    this.$set(detail, 'Net_cost', newNetUnitCost.toFixed(2));

    // Logging for debugging purposes
    console.log('Subtotal updated:', parsedSubtotal);
    console.log('The tax amount is:', totalTax);
    console.log('The new Net Unit Cost is:', newNetUnitCost.toFixed(2));
  ...
  Calcul_Total() {
    ...
    for (let i = 0; i < this.details.length; i++) {
      ...
      // Ensure Net_cost and quantity are parsed as floats for precision
      const netCost = parseFloat(this.details[i].Net_cost);
      const quantity = parseFloat(this.details[i].quantity);

      // Recalculate the subtotal
      this.details[i].subtotal = parseFloat((quantity * netCost + tax).toFixed(2)); // Round to 2 decimal places
    ...
    const totalWithoutDiscount = this.total - this.purchase.discount;

    // Calculate the total tax
    this.purchase.TaxNet = parseFloat(((totalWithoutDiscount * this.purchase.tax_rate) / 100).toFixed(2));

    // Calculate the grand total
    this.GrandTotal = totalWithoutDiscount + this.purchase.TaxNet + this.purchase.shipping;
