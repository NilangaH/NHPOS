...
class PosController extends BaseController
{
    ...
    public function GetProductsByParametre(request $request)
    {
        ...
        // Group by product_id and calculate total quantities
        $groupedProducts = $product_warehouse_data->groupBy('product_id')->map(function ($products) {
            $totalQty = $products->sum('qte');
            $product = $products->first();
            $product->qte = $totalQty;
            return $product;
        });

        $totalRows = $groupedProducts->count();

        // Apply pagination to the grouped products
        $groupedProducts = $groupedProducts->slice($offSet, $perPage);

        foreach ($groupedProducts as $product_warehouse) {
            if ($product_warehouse->product_variant_id) {
                $productsVariants = ProductVariant::where('product_id', $product_warehouse->product_id)
                    ->where('id', $product_warehouse->product_variant_id)
                    ->where('deleted_at', null)
