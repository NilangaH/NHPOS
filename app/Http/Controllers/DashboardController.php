...
class DashboardController extends Controller
{
    ...
    public function report_dashboard($warehouse_id, $array_warehouses_id)
    {
        ...
        ->whereNotNull('product_warehouse.exp_date')
        // ->whereNotNull('product_warehouse.created_at')
        ...
        ->orderBy('product_warehouse.exp_date')
        ...
        if ($product_warehouse_data->isNotEmpty()) {
            ...
            foreach ($product_warehouse_data as $product_warehouse) {
                if ($product_warehouse->qte <= $product_warehouse['product']->stock_alert) {
                    ...
                    $item['exp_date'] = $product_warehouse->exp_date;
        ...
        $product_warehouse_data_1 = product_warehouse::with('warehouse', 'product' ,'productVariant')
        // ->join('products', 'product_warehouse.product_id', '=', 'products.id')
        // ->where('manage_stock', true)
        // ->whereRaw('qte <= stock_alert')
        // ->whereNotNull('product_warehouse.exp_date')
        ...
        ->get();
        $stock_alert_1 = [];
        if ($product_warehouse_data_1->isNotEmpty()) {

            foreach ($product_warehouse_data_1 as $product_warehouse_1) {
                if ($product_warehouse_1->qte <= $product_warehouse_1['product']->stock_alert) {
                    if ($product_warehouse_1->product_variant_id !== null) {
                        $item_1['code'] = $product_warehouse_1['productVariant']->name . '-' . $product_warehouse_1['product']->code;
                    } else {
                        $item_1['code'] = $product_warehouse_1['product']->code;
                    }
                    $item_1['quantity'] = $product_warehouse_1->qte;
                    $item_1['exp_date'] = $product_warehouse_1->exp_date;
                    $item_1['purchase_id'] = $product_warehouse_1->purchase_id;
                    $item_1['pr_no'] = $product_warehouse_1->pr_no;
                    $item_1['name'] = $product_warehouse_1['product']->name;
                    $item_1['warehouse'] = $product_warehouse_1['warehouse']->name;
                    $item_1['stock_alert'] = $product_warehouse_1['product']->stock_alert;

                    $stock_alert_1[] = $item_1;
                }
            }
