...
class PurchasesController extends BaseController
{
    ...
    //                             ->where('product_id', $value['product_id'])
    //                             ->where('product_variant_id', $value['product_variant_id'])
    //                             ->where('exp_date', $value['exp_date'])
    //                             ->first();

    //                         // if ($unit && $product_warehouse) {
    //                         if ($unit) {
    //                             if ($product_warehouse) {
    //                                 if ($product_warehouse->exp_date == $value['exp_date']) {
    //                                     if ($unit->operator == '/') {
    //                                         $product_warehouse->qte += $value['quantity'] / $unit->operator_value;
    //                                     } else {
    //                                         $product_warehouse->qte += $value['quantity'] * $unit->operator_value;
    ...
    //                                         $product_warehouse->exp_date = $value['exp_date'];
        ...
        \DB::transaction(function () use ($request) {
            ...
            foreach ($data as $key => $value) {
                ...
                if ($order->statut == "received") {
                    ...
                    //     })
                    //     ->when($exp_date, function ($query, $exp_date) {
                    //         return $query->where('exp_date', $exp_date);
                    //     })
                    //     ->first();
                    // if ($unit) {
                    //     if ($product_warehouse) {
                    ...
                    //         $product_warehouse->product_id = $value['product_id'];
                    //         $product_warehouse->warehouse_id = $order->warehouse_id;
                    //         $product_warehouse->product_variant_id = $value['product_variant_id'] ?? null;
                    //         $product_warehouse->exp_date = $value['exp_date'] ?? null;
                    //         if ($unit->operator == '/') {
                    //             $product_warehouse->qte = $value['quantity'] / $unit->operator_value;
                    //         } else {
                    //             $product_warehouse->qte = $value['quantity'] * $unit->operator_value;
                    ...
                    $product_warehouse = new product_warehouse;
                    $product_warehouse->product_id = $value['product_id'];
                    $product_warehouse->purchase_id = $purchase_id;
                    $product_warehouse->pr_no = $pr_no;
                    $product_warehouse->warehouse_id = $order->warehouse_id;
                    $product_warehouse->product_variant_id = $value['product_variant_id'] ?? null;
                    $product_warehouse->exp_date = $value['exp_date'] ?? null;
                    if ($unit->operator == '/') {
                        $product_warehouse->qte = $value['quantity'] / $unit->operator_value;
                        $product_warehouse->order_qte = $value['quantity'] / $unit->operator_value;
                ...
                $orderDetails[] = [
                    ...
                    'total' => $value['subtotal'],
                    'imei_number' => $value['imei_number'],
                    'product_warehouse_id' => $product_warehouse ? $product_warehouse->id : null,
                ];
            }
...
// dd($prod_detail['exp_date']);
                    if($prod_detail['no_unit'] !== 0){
                        ...
                        if ($request['statut'] == "received") {
                            ...
                            if ($prod_detail['product_variant_id'] !== null) {
                                $product_warehouse = product_warehouse::where('deleted_at', '=', null)
                                    ...
                                    ->where('id',$prod_detail['id'])
                                    ->first();
                                    // if($prod_detail['exp_date'] )
                                        $product_warehouse->exp_date = $prod_detail['exp_date'];

                                    // }
                                if ($unit_prod && $product_warehouse) {
                                    if ($unit_prod->operator == '/') {
                                        $product_warehouse->qte += $prod_detail['quantity'] / $unit_prod->operator_value;
                                        $product_warehouse->order_qte += $prod_detail['quantity'] / $unit_prod->operator_value;
                        ...
                        $orderDetails['exp_date'] = $prod_detail['exp_date'];
    ...
    public function show(Request $request, $id)
    {
        ...
        // $purchase_data['exp_date'] = $purchase->details->exp_date;
