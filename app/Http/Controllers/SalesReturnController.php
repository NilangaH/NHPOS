                        ...
                        if ($unit && $product_warehouse) {
                            if ($unit->operator == '/') {
                                ...
                                foreach ($product_warehouse as $pw) {
                                    if ($quantity > 0 &&  $pw->qte < $pw->order_qte) {


                                        $orderQte = $pw->order_qte;
                                        $temp_s = $orderQte - $pw->qte;
                                        // $pw->qte -= $quantity / $old_unit->operator_value;
                                        // $quantity -= $pw->qte;
                                        if ($quantity < $temp_s) {
                                            $pw->qte += $quantity;
                                            $quantity = 0;
        ...
        \DB::transaction(function () use ($request, $id) {
            ...
            foreach ($new_return_details as $key => $product_detail) {
                ...
                if ($product_detail['no_unit'] !== 0 || $get_type_product == 'is_service') {
                    ...
                    $oldReturnedQty = SaleReturnDetails::where('id', $product_detail['id'])->first();
                    $ReturnedQty = $product_detail['quantity'] - $oldReturnedQty->quantity;
                    if ($ReturnedQty < 0) {
                        if ($request['statut'] == "received") {
                            ...
                            $product_warehouse_query = product_warehouse::whereNull('deleted_at')
                                ->where('warehouse_id', $request->warehouse_id)
                                ->where('product_id', $product_detail['product_id'])
                                ->where('qte', '>', 0);
                            ...
                            if ($product_detail['product_variant_id'] !== null) {
                                $product_warehouse_query->where('product_variant_id', $product_detail['product_variant_id']);
                            }
                            $product_warehouse = $product_warehouse_query->get();
                            if ($unit_prod && $product_warehouse) {
                                if ($unit_prod->operator == '/') {
                                    // $product_warehouse->qte -= $value['quantity'] / $unit_prod->operator_value;
                                    $quantity =  abs($ReturnedQty);
                                    ...
                                    foreach ($product_warehouse as $pw) {
                                        if ($quantity > 0) {
                                            ...
                                            if ($oldValue <= $quantity) {
                                                ...
                                                $pw->qte -= $quantity / $unit_prod->operator_value;
                                                $quantity -= $pw->qte;
                                            }
                                            $pw->save();
                                        } else {
                                ...
                                foreach ($product_warehouse as $pw) {
                                    if ($quantity > 0) {
                                        ...
                                        if ($oldValue <= $quantity) {
                                            ...
                                            $pw->qte -= $quantity / $unit->operator_value;
                                            $quantity -= $pw->qte;
                                        }
                                        $pw->save();
                                    } else {
